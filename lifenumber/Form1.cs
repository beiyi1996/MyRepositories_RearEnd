...
namespace lifenumber
{
    public partial class Form1 : Form
    {
        ...
        {
            string birth = dateTimePicker1.Value.Date.ToShortDateString();
            string birth_1 = ""; //用來存計算過後的生命靈數
            var mybirthday = new DateTime(dateTimePicker1.Value.Year, dateTimePicker1.Value.Month, dateTimePicker1.Value.Day); //存放使用者的生日用來判斷星座
            foreach (var i in birth) //計算生命靈數
            {
                if (i != '/') //跳過字元'/'
                {
                    birth_1 = birth_1 + i;
            ...
            int lifenumber = Countlifenumber(birth_1); //存判斷後的生命靈數
            string mystar = Creatstar(mybirthday); //存放判斷後使用者的星座
            var mystardictionary = new Dictionary<int, string>();
            mystardictionary = findlifenember(mystar);
            var lifewords = mystardictionary.Where((x) => x.Key == lifenumber);
            foreach (var item in lifewords)
            {
                label3.Text = "你的星座是：" + mystar + Environment.NewLine + "你的生命靈數 " + lifenumber.ToString() + " : " + item.Value;
            }
        }

        private int Countlifenumber(string birth)
        {
            int lifenumber = getbitrhsum(birth);
            if ( lifenumber >= 10)
            {
               lifenumber = Countlifenumber(lifenumber.ToString());
            }
            return lifenumber;
        }

        private int getbitrhsum(string birth)
        {
            int sum = 0;
        ...
        private string Creatstar(DateTime date)
        {
           var star1 = new List<Starmonth>();
            star1.Add(new Starmonth { Name = "牡羊座", startdate = new DateTime(date.Year,3,21), enddate = new DateTime(date.Year,4,20)} );
            star1.Add(new Starmonth { Name = "金牛座", startdate = new DateTime(date.Year, 4,21 ), enddate = new DateTime(date.Year, 5,21 )});
            star1.Add(new Starmonth { Name = "雙子座", startdate = new DateTime(date.Year, 5,22 ), enddate = new DateTime(date.Year, 6, 21)});
            star1.Add(new Starmonth { Name = "巨蟹座", startdate = new DateTime(date.Year, 6, 22), enddate = new DateTime(date.Year, 7, 23)});
            star1.Add(new Starmonth { Name = "獅子座", startdate = new DateTime(date.Year, 7, 24), enddate = new DateTime(date.Year, 8, 23)});
            star1.Add(new Starmonth { Name = "處女座", startdate = new DateTime(date.Year, 8, 24), enddate = new DateTime(date.Year, 9, 23)});
            star1.Add(new Starmonth { Name = "天枰座", startdate = new DateTime(date.Year, 9, 24), enddate = new DateTime(date.Year, 10, 23)});
            star1.Add(new Starmonth { Name = "天蠍座", startdate = new DateTime(date.Year, 10, 24), enddate = new DateTime(date.Year, 11, 22)});
            star1.Add(new Starmonth { Name = "射手座", startdate = new DateTime(date.Year, 11, 23), enddate = new DateTime(date.Year, 12, 22)});
            star1.Add(new Starmonth { Name = "魔羯座", startdate = new DateTime(date.Year, 12, 23), enddate = new DateTime(date.Year, 1, 20)});
            star1.Add(new Starmonth { Name = "水瓶座", startdate = new DateTime(date.Year, 1, 21), enddate = new DateTime(date.Year, 2, 19)});
            star1.Add(new Starmonth { Name = "雙魚座", startdate = new DateTime(date.Year, 2, 20), enddate = new DateTime(date.Year, 3, 20)});
            var starname = star1.Find((x) => x.startdate <= date && x.enddate >= date);
            return starname.Name;
        }

        private static Dictionary<int,string> findlifenember(string mystar)
        {
            var result = new Dictionary<int, string>();
            switch (mystar)
            {
